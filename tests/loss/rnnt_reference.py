"""Pure-PyTorch RNN-T loss reference.

The exact, ``gradcheck``-verified oracle for the fused CUDA kernel in
``asr.loss.rnnt``. It materializes the dense ``(B, T, U, V)`` joint tensor, so it
is only used to validate correctness in the test suite, never in training.
"""
