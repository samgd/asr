...
def main(cfg: DictConfig) -> None:
    ...
    if cfg.dataset._target_.endswith("BucketDataset"):
        ...
        dataset = hydra.utils.instantiate(cfg.dataset, datasets=inner, normalize=normalize, _recursive_=False)
    else:
        dataset = hydra.utils.instantiate(cfg.dataset, normalize=normalize, tokenizer=tokenizer)
