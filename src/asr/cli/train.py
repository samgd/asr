...
def main(cfg: DictConfig) -> None:
    normalization = hydra.utils.instantiate(cfg.normalization)
    ...
    if cfg.dataset._target_.endswith("BucketDataset"):
        ...
        dataset = hydra.utils.instantiate(cfg.dataset, datasets=inner, normalization=normalization, _recursive_=False)
    else:
        dataset = hydra.utils.instantiate(cfg.dataset, normalization=normalization, tokenizer=tokenizer)

    loader = hydra.utils.instantiate(cfg.dataloader, dataset=dataset)
    eval_dataset = hydra.utils.instantiate(cfg.eval_dataset, normalization=normalization, tokenizer=tokenizer)
