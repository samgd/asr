...
class TDTSystem(torch.nn.Module):
    ...
    def evaluate(self, loader: Iterable[Batch], logger: Logger, step: int, device: str | torch.device) -> None:
        ...
        losses: list[float] = []
        edits, ref_words = 0, 0
        durations: list[int] = []  # every argmax duration the decoder chose, across the eval set
        for batch in loader:
        ...
        if durations:
            logger.distribution("duration", durations, step=step, stage="eval")
